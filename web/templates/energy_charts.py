...
(function() {{
  ...
  var layout  = {{
    ...
    xaxis: {{
      ...
      showgrid: false, tickfont: {{color: '#6b7080'}}
    }},
    yaxis:  {{title:'kWh',   showgrid:true,  gridcolor:'#2a2d3a', titlefont:{{size:11}}, tickfont:{{color:'#6b7080'}}}},
    yaxis2: {{title:'£/kWh', overlaying:'y', side:'right', rangemode:'tozero', showgrid:false, titlefont:{{size:11}}, tickfont:{{color:'#6b7080'}}}},
    legend: {{
      orientation: 'h',
...
.channel-header td {{ font-size: 12px; color: #6b7080; padding-bottom: 4px; }}
...
.scol.sub    {{ color: #8b91a8; }}
.scol.export {{ color: #f59e0b; }}
...
.scol.export .slabel {{ color: #d97706; }}
...
.sval.export-val {{ color: #f59e0b; }}
.sval.sub-val    {{ color: #8b91a8; font-size: 11px; }}
.scol.export .sval {{ color: #f59e0b; }}
...
.rate-row.export {{ color: #92660a; }}
.rate-row.sub    {{ color: #4b5060; }}
...
def generate_net_heatmap(blocks):
    ...
    def make_colorscale(mn, mx):
        """Return a valid 7-stop colorscale with dark surface at zero, handling all-positive or all-negative ranges."""
        wp = max(0.0, min(1.0, (0 - mn) / (mx - mn)))
        if wp <= 0.01:
            # All positive (import only) — blue-white-orange/red ramp from min to max
            return [[0.0, "#1a1d27"], [0.33, "#ffcc99"], [0.66, "#ff6600"], [1.0, "#cc0000"]]
        elif wp >= 0.99:
            # All negative (export only) — blue ramp
            return [[0.0, "#003366"], [0.33, "#0066cc"], [0.66, "#00aa66"], [1.0, "#1a1d27"]]
        else:
            c1 = round(wp * 0.33, 4)
            c2 = round(wp * 0.66, 4)
            c3 = round(wp, 4)
...
html, body {{ margin:0; padding:0; }}
  ...
  #scroll {{ width:{heatmap_width}px; height:{div_height}px; overflow-y:auto; border:1px solid #2a2d3a; }}
</style>
<body>
<div id="outer">
  <div id="scroll">
...
{{
  ...
  y: {y_json},
  type: 'heatmap',
  colorscale: [[0, 'rgba(0,0,0,0)'], [1, 'rgba(255,255,255,0.04)']],
  zmin: 0, zmax: 1,
  showscale: false,
  hoverinfo: 'skip'
}}
...
var layout = {{
  title: {{text: 'Net Energy Flow', x: 0.5, font: {{color: '#e8eaf0'}}}},
